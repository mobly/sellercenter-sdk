...
{
    /**
     * A waiter associated with an API resource (e.g. EC2 instance) that waits
     ...
     * for wait completion.
     *
     * @param SdkClientInterface $client Client used to execute commands
    ...
    private function checkErrorAcceptor(SdkException $e, array $config)
    {
        if ($config['success_type'] == 'error'
            && $e->getApiErrorCode() == $config['success_value']
        ) {
            // Mark as a success
            return true;
        }
